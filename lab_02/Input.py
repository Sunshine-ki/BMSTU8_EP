...
class InputList(tk.Frame):
    def __init__(self, master, items = [], title=None):
        ...
        i = 0
        if title:
            t = tk.Label(self, text=title, background=BACKGROUND)
            t.grid(row=i, column=0, columnspan=2, padx=10, pady=10)
            i += 1
        for item in items: 
            label = tk.Label(self, text=item.text, background=BACKGROUND)
            label.grid(row=i, column=0, sticky="e")
            entry = tk.Entry(self, width=10, textvariable=item.var, background=BACKGROUND)
